...
assert.match(multiplayerWorkerSource, /reservedPlayerIds\(\)[\s\S]*?new Set[\s\S]*?effectiveHumanCount\(\)[\s\S]*?this\.effectiveHumanCount\(\) >= MAX_MATCH_PLAYERS && !resumed/, "deduplicated disconnected identities reserve capacity without allowing a seventeenth human to resume");
