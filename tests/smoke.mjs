...
assert.match(mainSource, /startMatch\(welcomeOverride = null, loadingPainted = false\)\s*\{\s*if \(!welcomeOverride && !loadingPainted\) return this\.queueMatchStart\(false\)/, "local launches paint their loader while server-started lobbies preserve their connection");
assert.match(mainSource, /queueMatchStart\(sameSeed = false\)[\s\S]*?requestAnimationFrame[\s\S]*?this\.startMatch\(null, true\)/, "matches restart in memory without reloading the engine");
assert.match(mainSource, /resumePendingMatch\(\)[\s\S]*?sessionStorage\.removeItem\(MATCH_SESSION_KEY\)[\s\S]*?this\.startMatch\(\)/, "legacy reload tickets are still consumed exactly once");
...
assert.match(mainSource, /resumePendingMatch[\s\S]*?clampMatchMinutes\(saved\.timeLimitMinutes\)/, "legacy reload recovery preserves the selected time limit");
...
assert.match(multiplayerWorkerSource, /timeLimitMinutes = clampMatchMinutes\(input\.timeLimitMinutes\)[\s\S]*?open:\$\{targetSize\}:\$\{difficulty\}:\$\{timeLimitMinutes\}[\s\S]*?endsAt: mode !== "quick" \? 0 : now \+ 4_000 \+ timeLimitMinutes \* 60_000/, "online matchmaking separates durations and makes the room server authoritative for the chosen limit");
...
assert.match(multiplayerClientSource, /directRoomCode[\s\S]*?reconnect\(detail\)[\s\S]*?resumeToken[\s\S]*?dispatchEvent\(new CustomEvent\("reconnected"/, "an interrupted match resumes its room and authoritative player identity before surfacing a fatal disconnect");
assert.match(multiplayerClientSource, /ROOM_SESSION_PREFIX[\s\S]*?storedRoomSession[\s\S]*?rememberRoomSession[\s\S]*?options\.mode === "private"/, "private-room credentials persist on the player's device for deliberate leave-and-rejoin recovery");
assert.match(multiplayerClientSource, /reconnectDelays[\s\S]*?resume_ack[\s\S]*?globalThis\.navigator\?\.onLine === false/, "resume rotation is acknowledged after local persistence and offline handoffs do not burn retry attempts");
assert.match(multiplayerClientSource, /startHeartbeat\(socket\)[\s\S]*?Heartbeat timeout[\s\S]*?\{"type":"ping"\}/, "the multiplayer socket detects half-open network paths with a bounded heartbeat");
...
assert.match(multiplayerWorkerSource, /phase: mode !== "quick" \? "lobby" : "playing"[\s\S]*?handleLobbyStart[\s\S]*?rosterMessage\("match_start"\)[\s\S]*?this\.meta\.phase = "lobby"[\s\S]*?rosterMessage\("lobby"\)/, "the room authority starts private matches from the lobby and returns the whole room after each result");
...
assert.match(mainSource, /reconnecting[\s\S]*?handleNetworkReconnect[\s\S]*?syncOnlineRoster/, "brief network interruptions recover in place without destroying the active arena");
assert.match(mainSource, /networkRecovering = true[\s\S]*?clearTransientNetworkCombat\(\)[\s\S]*?authoritativePosition[\s\S]*?networkRecovering\)[\s\S]*?updateAudio/, "recovery freezes speculative play and reconciles the controlled fighter to the authoritative room snapshot");
assert.match(mainSource, /clearMatch\(preserveNetwork = false\)[\s\S]*?networkRecovering = false[\s\S]*?hideNetworkReconnecting\(\)/, "leaving a recovering match cannot freeze the next training or online session");
assert.match(multiplayerWorkerSource, /setWebSocketAutoResponse[\s\S]*?resumableSession[\s\S]*?rememberDisconnectedPlayer/, "the room keeps heartbeat handling hibernation-safe and preserves short-lived reconnect sessions");
assert.match(multiplayerWorkerSource, /PRIVATE_MATCH_REJOIN_GRACE_MS[\s\S]*?this\.meta\.endsAt[\s\S]*?priorResumeToken[\s\S]*?crypto\.randomUUID\(\)/, "private-match identities remain reclaimable through match end while every successful resume rotates its bearer credential");
assert.match(multiplayerWorkerSource, /RESUME_ACK_GRACE_MS[\s\S]*?handleResumeAck[\s\S]*?session\?\.player\?\.id !== player\.id/, "the prior reconnect credential remains a tightly bounded alias until the rotated credential is acknowledged");
assert.match(multiplayerWorkerSource, /handleLobbyStart[\s\S]*?session\.player = resetRoomPlayer[\s\S]*?checkMatchEnd[\s\S]*?session\.player = resetRoomPlayer/, "disconnected private-room sessions reset and retime across both match start and match completion");
assert.match(multiplayerWorkerSource, /reservedPlayerIds\(\)[\s\S]*?new Set[\s\S]*?effectiveHumanCount\(\)[\s\S]*?this\.effectiveHumanCount\(\) >= capacity && !resumed/, "deduplicated disconnected identities reserve capacity without allowing a seventeenth human to resume");
...
assert.equal(ARENA_PORTAL_PAIRS.length, 2, "client and server share the two authoritative portal pairs");
assert.equal(isArenaPortalTransition({ x: -96, y: 0, z: 0 }, { x: 0, y: 66.35, z: -10 }), true, "a known paired portal transition is authorized");
assert.equal(isArenaPortalTransition({ x: -96, y: 0, z: 0 }, { x: 96, y: .35, z: 0 }), false, "a long jump to an unpaired portal exit is rejected");
const undersideWorld = new ArenaWorld(new THREE.Scene(), "BLAST-01");
const undersidePrevious = new THREE.Vector3(-60.54931264137849, 11.409479528665543, -45.15553917037323);
const undersidePosition = new THREE.Vector3(-59.141429091411005, 11.874217872694135, -45.55605216224315);
const undersideCollision = undersideWorld.resolve(undersidePosition, .72, undersidePrevious);
assert.equal(undersideCollision.ceiling, true, "an upward capsule entering the underside tolerance band is clamped below the deck instead of pushed into its neighboring tile");
assert.ok(undersidePosition.y < undersidePrevious.y && Math.abs(undersidePosition.z + 45.55605216224315) < 1e-9, "underside collision resolves vertically without trapping the capsule across a platform seam");
const alreadyTrappedPosition = new THREE.Vector3(-59.141429091411005, 11.874217872694135, -48.72);
const trappedCollision = undersideWorld.resolve(alreadyTrappedPosition, .72, alreadyTrappedPosition.clone());
assert.equal(trappedCollision.ceiling, true, "a capsule already embedded beneath an adjacent deck tile is ejected on its next collision update");
assert.ok(alreadyTrappedPosition.y < undersidePrevious.y, "an existing underside trap recovers vertically instead of ping-ponging between neighboring collision boxes");
const recoveredPosition = alreadyTrappedPosition.clone();
for (let update = 0; update < 100; update++) undersideWorld.resolve(alreadyTrappedPosition, .72, alreadyTrappedPosition.clone());
assert.ok(alreadyTrappedPosition.equals(recoveredPosition), "the recovered capsule remains stable across repeated collision updates");
for (const portal of undersideWorld.portals) {
  const remote = { position: portal.position.clone(), previousPosition: new THREE.Vector3(), radius: .72 };
  const exit = portal.pair.position.clone().setY(portal.pair.position.y + .35);
  reconcileRemotePosition(remote, exit, .25, undersideWorld);
...
assert.match(mainSource, /applyNetworkRespawn\(message\)[\s\S]*?respawnDisposition[\s\S]*?networkRespawnRequests\.delete[\s\S]*?if \(disposition === "duplicate"\) return;[\s\S]*?player\.respawn[\s\S]*?player\.ammo/, "a duplicate respawn only refreshes lifecycle metadata and cannot teleport, refill, or reset the living fighter");
assert.match(mainSource, /applyNetworkRespawnPending\(message\)[\s\S]*?serverRemainingSeconds\(message\.respawnAt, message\.serverTime\)/, "pending respawn countdowns use server-relative time");
assert.match(mainSource, /advanceRespawnRetry[\s\S]*?request\.shouldRecover[\s\S]*?multiplayer\.recover/, "four seconds of respawn silence starts the in-place session resume path");
...
assert.match(multiplayerClientSource, /lifeState: 1[\s\S]*?lifeSequence: player\.networkLifeSequence[\s\S]*?respawnId: player\.networkRespawnId/, "capable clients advertise lifecycle support and tag movement with the accepted life");
...
assert.match(multiplayerWorkerSource, /lifeStateCapable = url\.searchParams\.get\("lifeState"\) === "1"[\s\S]*?updatePlayerState\(player, state, now, requireLifeState = false\)/, "the additive lifecycle capability retains protocol-v1 compatibility while enabling stricter state validation");
assert.match(multiplayerWorkerSource, /state\.respawnId !== player\.respawnId[\s\S]*?player\.awaitingRespawnState[\s\S]*?RESPAWN_SPAWN_TOLERANCE/, "capable post-respawn state must match both the canonical spawn envelope and opaque life identifier");
assert.match(multiplayerWorkerSource, /authoritativeSpawnPoint[\s\S]*?pillarDrop[\s\S]*?Math\.abs\(position\.y - spawn\.y\) > RESPAWN_VERTICAL_TOLERANCE[\s\S]*?playerCapsuleIntersectsStructure/, "the first accepted life state follows the current structural spawn height and cannot claim a sky, ground, or embedded position");
assert.match(multiplayerWorkerSource, /respawnMessage\(player\)[\s\S]*?respawn_replayed[\s\S]*?type: "respawn_pending"/, "the room acknowledges early requests and replays the immutable accepted respawn instead of mutating twice");
...
const remoteAcrossCover = { position: ropeStart.clone(), previousPosition: new THREE.Vector3(), radius: .72 };
reconcileRemotePosition(remoteAcrossCover, ropeEnd, .25, worldB);
assert.ok(remoteAcrossCover.position.equals(ropeEnd), "remote reconciliation snaps to the authoritative side when interpolation would visibly cross solid cover");
for (const portal of worldB.portals) {
  const remote = { position: portal.position.clone(), previousPosition: new THREE.Vector3(), radius: .72 };
  const exit = portal.pair.position.clone().setY(portal.pair.position.y + .35);
  reconcileRemotePosition(remote, exit, .25, worldB);
  assert.ok(remote.position.equals(exit), "every large portal displacement snaps to its authoritative exit in the SAME-SEED layout");
}
const remoteInOpen = { position: new THREE.Vector3(92, 10, 70), previousPosition: new THREE.Vector3(), radius: .72 };
reconcileRemotePosition(remoteInOpen, new THREE.Vector3(98, 10, 70), .5, worldB);
assert.ok(remoteInOpen.position.equals(new THREE.Vector3(95, 10, 70)), "remote reconciliation keeps smooth interpolation through unobstructed space");
