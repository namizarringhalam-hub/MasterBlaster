...
Object.assign(game, { scene, paused: true, mode: "training", players: [original], projectiles: [], hazards: [], effects: [], decoys: [],
  ...
  combatVisuals: new CombatVisuals(scene), sound: new Proxy({}, { get: () => () => {} }), audioSpatial: () => ({}),
...
const run = async options => {
  const frames = [], beforeQueries = collisionQueries, beforeTerrain = terrainCalls;
  await withWallImpactReview(game, options, async state => { assert.equal(Math.random, globalRng); frames.push(structuredClone(state)); });
  ...
  assert.equal(frames[0].rings.length, 1); assert.equal(frames[0].impact.suppliedNormal, null);
...
const elements = { "camera-captures": { replaceChildren: () => { links.length = 0; }, append: link => links.push(link) },
  "wall-angle": { value: "normal" }, "wall-turn": { checked: false }, "wall-gameplay": { checked: true }, "wall-contact": { checked: false } };
