...
export async function withStructuralDustReview(game, { gameplay = false, diagnostic = "hide" } = {}, capture) {
  ...
  const layerState=(mesh,indices)=>indices.map(index=>({index,matrix:Array.from(mesh.instanceMatrix.array.slice(index*16,index*16+16)),
    color:mesh.instanceColor?Array.from(mesh.instanceColor.array.slice(index*3,index*3+3)):null}));
