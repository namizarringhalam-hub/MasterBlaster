...
export function surfaceTextures(seed, repeat) {
  const size = 256, tile = 64, seedValue = seedFromText(seed);
  ...
  for (let y = 0; y < tile; y++) for (let x = 0; x < tile; x++) {
    ...
    const rivet = Math.hypot(Math.min(Math.abs(x - 6), Math.abs(x - 57)), Math.min(Math.abs(y - 6), Math.abs(y - 57)));
    heights[y * tile + x] = Math.min(1, edge / 2.4) * .45 + Math.max(0, 1 - rivet / 1.6) * .25;
  }
  ...
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    ...
    const grain = hash(x, y) * 3 + hash(Math.floor(x / 12), y) * 2;
    const variation = hash(Math.floor(x / tile), Math.floor(y / tile)) * 12;
    const albedo = 154 + h * 52 + variation + grain;
    ...
    const roughness = h < .4 ? 230 : 207 + grain;
