...
3. Real-time reflections — implemented, validating final browser cleanup: native WebGPU high half-resolution SSR from current beauty/depth/normal/material attachments; roughness blur, bounded reach and intensity, edge fade, existing IBL fallback. WebGL/medium/low retain environment reflections. No separate planar pass: the dry arena has no mirror/water plane. Full suite and build/hosting pass; actual SSR/blur WGSL generation added. Native on/off visual comparison and cover destruction passed. Boot index-CWzpAAp8.js.
4. Wet surfaces/puddles — pending.
